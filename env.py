...
class LNEnv(Env):
    def __init__(self, G, transactions, train=True) -> None:
        ...
        self.action_space = spaces.Box(
            low=-1, high=1, shape=(self.observation_size + 1,), dtype=np.float32
        )
    ...
    def step(self, action):
        ...
        while True:
            neighbors = [
                n
                for n in action
                if n in self.g.neighbors(self.path[-1]) and n not in self.path
            ]
            if len(neighbors):
        ...
        for i in range(1, len(self.path) - 1):
            if self.g.has_edge(self.path[i], self.path[i + 1]):
                ...
                total_cost += cost_function(
                    self.g, self.path[i], self.path[i + 1], self.amount
                )
        return -total_cost
